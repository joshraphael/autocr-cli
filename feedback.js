...
function generate_code_note_stats(ach, notes, rp)
{
	let stats = {};
	...
	let asset_addresses = [...ach.getAchievements().map(e => e.logic.getAddresses()),
		...ach.getLeaderboards().map(e => e.components).flatMap(cmps => Object.values(cmps)).map(cmp => cmp.getAddresses())
	];
...
function* check_pointers(args)
{
	...
	for (const [gi, g] of logic.groups.entries())
	{
		for (const [ri, req] of g.entries())
		{
			...
			const note = get_note(req.lhs.value, codeNotes);
			if (!note) continue;
