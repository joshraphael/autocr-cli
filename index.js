...
const { AchievementSet, CodeNote, RichPresence, CodeNoteSet } = require("./achievements");
...
try
{
    ...
    for (const obj of notes) {
        if (obj.Note) {
            codeNotes.add(new CodeNote(obj.Address, obj.Note, obj.User));
        }
    }
}
catch (e)
...
assess_code_notes(achievements, codeNotes, rp)
...
for( var i = 0; i < codeNotes.feedback.issues.length; i++ ) {
    for( var k = 0; k < codeNotes.feedback.issues[i].length; k++ ) {
        ...
        // grouping FAIL and ERROR together, might need to seperate later on
        if( issue.type.severity === FeedbackSeverity.FAIL || issue.type.severity === FeedbackSeverity.ERROR ) {
